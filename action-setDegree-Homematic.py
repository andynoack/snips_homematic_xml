...
def action_wrapper(hermes, intentMessage, conf):
    ...
    try:
        ...
        spoken_degree = intentMessage.slots.Grad.first().value
        if common.changeDeviceState(url, common.getID(dl, spoken_name), spoken_degree):
            if verbose == 'True' or verbose == 'true':
                result_sentence = "Setze " + spoken_name + " auf " + str(spoken_degree) + " Grad."
            else:
                result_sentence = "OK"
        else:
...
if __name__ == "__main__":
    with Hermes("localhost:1883") as h:
        h.subscribe_intent("ndy1982:setDegree", subscribe_intent_callback) \
         .start()
